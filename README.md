...
This version contains first release of the cone geometry in reflection,
which is a different flavor of the XSPEC model "stokes_torus" v2.0. It
does not contain the trTheta parameter, but provides Theta in degrees
directly.

Refer to Podgorný J. (2025) regarding the definition and model behavior
of all parameters in this release.
...
half-opening angle of the double-cone reflector (see Podgorný J. et al., 2024,
for details, or below), the skew of the inner walls, the ionization parameter
xi0 at the inner edge of the reflector, the ionization profile's power-law index
...
normalized according to the distance of the reflector from the center,
the distance to the source, and the expected density at the inner edge.
...
switching between "pol_deg" = -1 and 0 <= "pol_deg" <= 1; however within the
second mode one can interpolate and fit without extra memory consumption.
In the future, we would like to restore the B switch that allows to remove
reflection below the double-cone equator. Currently, all configurations are
computed for B = 1 (with reflection from below the equator included, which matters
only for polarization in certain regions of the parameter space).

For any issues regarding the use of stokes_cone, please, contact J. Podgorný at
...
* **par3 ... Theta**
  - cone half-opening angle in degrees
* **par4 ... rhorhoin**
  - the skew rho, in the units of rho_in, of the inner walls of the cone
    between 1 and 10
* **par5 ... xi0**
  - the ionization parameter in erg cm s^{-1} at the inner-most ring of
    the cone in the equatorial plane
    ...
    -  xi0 < 0 - it assumes fully ionized reflection with 100% albedo and uses
       ...
       the distance between the cone and the center;
...
* **par13 ... norm**
  - equal to (100*rho_in_pc/D_Mpc)^2 * (n_H0/1e15) where rho_in is the physical
    distance between the center and the cone in pc, D_Mpc is the distance to the
    source in Mpc, and n_H0 is the expected density at the inner edge of the cone
    in cm^{-3};
...
* **rho_in_pc**
  - if one re-enters the current normalization value as XSET variable "NORMVAL", if one
    ...
    specifies the expected density at the inner edge of the cone in cm^{-3} as XSET
    variable "NH0", then the model provides the distance rho_in between the cone and
    the center in pc
* **rho_in_r_g**
  - if one additionally specifies the central object's mass as XSET variable "MASS", then
    the model provides the distance rho_in between the cone and the center in r_g = GM/c^2
* **RF**
...
* **reprocessing tables**
  ...
  opaque cone-shaped structure, more details in Podgorný J. et al. (2024), Podgorný J. (2025))
  - [32287641.zip](https://doi.org/10.6084/m9.figshare.32287641)
...
1. **Download the source code files**
   into a directory where you want to install the model, e.g. '/path/to/stokes_cone-master/'

2. **Download the reprocessing tables in FITS files**
   [32287641.zip](https://doi.org/10.6084/m9.figshare.32287641)
   with the tables in a directory called "tables" within the directory with stokes_cone,
   i.e. '/path/to/stokes_cone-master/tables'; not all tables are required if one uses only
...
4. **Compile the code** inside XSPEC (needed to be done only once):
   ...
   '/path/to/stokes_cone-master'):

   `initpackage stcone lmodel-stcone.dat /path/to/stokes_cone-master`

   **Note**:
   Your XSPEC installation must have been originally installed from the source 
   code distribution. Local models, like stokes_cone, cannot be used if the XSPEC
   was originally installed from the pre-compiled binary distribution.

5. **Load the stokes_cone model** into XSPEC:

   To use the stokes_cone model inside XSPEC, first the model package needs to be
   loaded and also setup a directory containing the stokes_cone set:

   `lmod stcone /path/to/stokes_cone-master`
   `xset XSDIR /path/to/stokes_cone-master`

6. Then the **stokes_cone model may be used** in the usual way, e.g.:
...
of, these data sets. To overcome these disadvantages, the stokes_cone model provides
...
4. **Set stokes_cone par12 to -1**

   To be able to use the traditional way of using the polarisation data with the model in XSPEC, one needs to set the stokes_cone parameter par12 to -1:
